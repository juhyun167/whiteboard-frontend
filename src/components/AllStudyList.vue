  ...
  <div class="component-wrapper">
    ...
    <div class="helper-text-wrapper">
      <div class="helper-text">
        {{ data.studyList.length }}개의 개설된 <strong>스터디</strong>가
        있습니다.
      </div>
      <div class="paginator-button-wrapper">
        <a
          v-if="!paginator.isFirstPage"
        ...
        >
          <Card class="shadow-3">
            ...
            <template #title>
              ...
              <Tag v-if="study.isTeacher" class="status status-teacher"
                >교육중</Tag
              >
              <Tag v-if="study.isStudent" class="status status-student"
                >수강중</Tag
              >
            </template>
            <template #content>
              <Button
                v-if="!study.isStudent"
                label="수강 신청"
                @click.prevent="enrollStudy(study.id)"
                icon="pi pi-check"
                class="p-button-enroll"
              />
              <Button
                v-else
                label="수강 취소"
                @click.prevent="unrollStudy(study.id)"
                icon="pi pi-times"
                class="p-button-enroll"
              />
            </template>
...
export default {
  ...
  setup(props, { emit }) {
    ...
    const paginator = reactive({
      ...
      thisPageItemCount: computed(() =>
        paginator.itemsPerPage * paginator.thisPage < data.studyList.length
          ...
          : data.studyList.length -
            paginator.itemsPerPage * (paginator.thisPage - 1)
      ),
      ...
      isLastPage: computed(
        () => paginator.thisPage === paginator.totalPageCount
      ),
      toPrevPage: () => {
        paginator.thisPage = paginator.prevPage;
      },
      toNextPage: () => {
        paginator.thisPage = paginator.nextPage;
      },
    });
    ...
    const getAllStudyList = async () => {
      ...
      if (result.success) {
        ...
        if (result.status) {
          ...
          errorMessage =
            "스터디 목록을 가져오지 못했습니다. 인터넷 상태를 확인해주세요.";
        }
        toast.add({
          severity: "error",
          summary: "요청이 실패했습니다.",
        ...
        if (result.status) {
          if (result.status < 500) {
            if (result.status === 400) {
              errorMessage =
                "존재하지 않는 스터디입니다. 버튼을 통해 올바르게 신청해주세요.";
          ...
          errorMessage =
            "수강신청 요청이 실패했습니다. 인터넷 상태를 확인해주세요.";
        }
        toast.add({
          severity,
    ...
    const unrollStudy = async (studyId) => {
      ...
      if (result.success) {
        ...
        if (result.status) {
          ...
          errorMessage =
            "수강취소 요청이 실패했습니다. 인터넷 상태를 확인해주세요.";
        }
        toast.add({
          severity,
